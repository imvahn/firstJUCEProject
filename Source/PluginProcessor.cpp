...
void SimpleEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    // max number of samples it will process at one point
    spec.maximumBlockSize = samplesPerBlock;
    // number of channels it will process
    spec.numChannels = 1;
    // sample rate
...
void SimpleEQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // block that represents our individual channel
...
//    osc.process(stereoContext);
    ...
    // context that provides a wrapper around the block so the chain can use it
    juce::dsp::ProcessContextReplacing<float> leftContext(leftBlock);
    juce::dsp::ProcessContextReplacing<float> rightContext(rightBlock);
    // pass contexts to auto filter chain
    leftChain.process(leftContext);
    rightChain.process(rightContext);
    
    // update single channel fifo
...
void SimpleEQAudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    ...
    // write plugin state (apvts) to memory
    // ------------------------------------
    // set up the stream and connect to the memoryblock
    juce::MemoryOutputStream mos(destData, true);
    // write apvts (plugin) state to the stream
...
void SimpleEQAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    // retrieve plugin state (apvts) from memory
    // -----------------------------------------
    // retrieve tree
    auto tree = juce::ValueTree::readFromData(data, sizeInBytes);
    if ( tree.isValid() )
    {
        // replace the state with state stored in memory
        apvts.replaceState(tree);
        // update filters with saved parameter values
        updateFilters();
    }
}
...
//    apvts.getParameter("LowCut Freq")->getValue(); // can't use this function because all values are normalized
...
juce::AudioProcessorValueTreeState::ParameterLayout SimpleEQAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID("LowCut Freq", 1),
                                                           "LowCut Freq", // low end of range, high end of range, step, skew, default
